...
1.  **纯代码 GUI 架构 (Programmatic GUI)：**
    完全脱离 GUIDE/App Designer 依赖，通过编写底层代码直接管理 `Figure`、`UIPanel` 和 `UIControl` 对象。这种方式不仅实现了界面的完全可控（如自适应布局），还加深了对 MATLAB 图形句柄系统（Handle Graphics）的理解。

2.  **事件驱动的响应式编程 (Event-Driven Programming)：**
